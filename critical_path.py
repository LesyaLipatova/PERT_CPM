...
class CriticalPath:
    ...
    def algoritmCPM(dataMatrix, nodes):
                ...
                if len(predecesor) > 0:
                    if s in matrix:
                        ...
                        if matrix[predecesor[0]]['earlyFinish'] > matrix[s]['earlyStart']:
                            ES = int(matrix[predecesor[0]]['earlyFinish'])
                        else:
                            ES = int(matrix[s]['earlyStart'])
                    else:
    ...
    def algoritmPERT(dataMatrix, nodes):
        visited = []  # List to keep track of visited nodes.
        queue = []  # Initialize a queue
        matrix = {}  # Initialize forward pass table structure
        matrix2 = {}  # Initialize backward pass table structure
            ...
            while queue:
                ...
                if len(predecesor) > 0:
                    if s in matrix:
                        # the EF of the predecessors have to be compared to select the bigger value, this will be the ES of the node that we are visiting
                        if matrix[predecesor[0]]['earlyFinish'] > matrix[s]['earlyStart']:
                            ES = round(matrix[predecesor[0]]['earlyFinish'], 2)
                        else:
                            ES = round(matrix[s]['earlyStart'], 2)
                    else:
                        ES = round(matrix[predecesor[0]]['earlyFinish'], 2)
                    element = {'earlyFinish': EF + duraciones[s], 'earlyStart': ES,
                               'lateStart': '-', 'lateFinish': '-', 'slack': '-'}
            ...
            while queue:
                ...
                if len(predecesor) > 0:
                    if matrix2[s]['lateFinish'] != 0:
                        ...
                        # the LS of the node is compared to select the smaller value to be the LF of the visited node
                        if matrix2[predecesor[0]]['lateStart'] < matrix2[s]['lateFinish']:
                            LF = round(matrix2[predecesor[0]]['lateStart'], 2)
                        else:
                            LF = round(matrix2[s]['lateFinish'], 2)
                    else:
                        LF = round(matrix2[predecesor[0]]['lateStart'], 2)
                    LS = round(LF, 2) - round(duraciones[s], 2)
                    slack = round(LF, 2) - round(matrix[s]['earlyFinish'], 2)
                    element = {'earlyStart': matrix[s]['earlyStart'], 'earlyFinish': matrix[s]
                    ['earlyFinish'], 'lateStart': round(LS, 2), 'lateFinish': round(LF, 2), 'slack': round(slack, 2)}
                else:
                    LF = round(matrix[s]['earlyFinish'], 2)
                    LS = round(LF, 2) - round(duraciones[s], 2)
                    slack = round(LF, 2) - round(matrix[s]['earlyFinish'], 2)
                    element = {'earlyStart': matrix[s]['earlyStart'], 'earlyFinish': matrix[s]
                    ['earlyFinish'], 'lateStart': round(LS, 2), 'lateFinish': round(LF, 2), 'slack': round(slack, 2)}
    ...
    def procesarInput(self, listaInput):
        ...
        for i in range(len(listaInput)):
            ...
            for j in range(columns):
                ...
                if (j == 1):
                    matrix_data[i][j] = tuplas[j]
                if (j == 2):
                    matrix_data[i][j] = round(tuplas[j], 2)
                if (j == 3):
        ...
        return self.algoritmCPM(matrix_data, nodes)

    #####################################################################

    def processArchivoCPM(self, archivoProc):
    ...
    def processArchivoPERT(self, archivoProc):
        df = archivoProc.fillna("")
        # to know how many columns and rows, we will use shape
        rows = df.shape[0]
        columns = df.shape[1]
        ...
        for i in range(rows):
            for j in range(columns):
                ...
                if (j == 3):
                    matrix_data[i][j] = str(df["predecessors"][i]).split(sep=",")
        return self.algoritmPERT(matrix_data, nodes)